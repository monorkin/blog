...
module HeaderNavHelper
  ...
  def desktop_header_nav(&block)
    content_tag(:nav,
                class: 'hidden md:flex py-2 lg:py-4 px-2 lg:px-4 flex-row items-center gap-x-4 justify-between') do
    ...
    content_tag(:nav, class: 'flex md:hidden py-2 px-2 flex-row items-center gap-x-4 justify-between') do
      content_tag(:ul) do
        ...
        content_tag(:div, data: { controller: 'navigation', navigation_dialog_outlet: '#navigation-dialog' }) do
          content_tag(:a, class: 'round text-indigo-600 dark:text-yellow-500 cursor-pointer',
                          data: { action: 'navigation#openMenu' }) do
            menu_icon(class: 'w-6 h-6')
          end +
            content_tag(
              :dialog,
              class: 'aboslute inset-5 w-full h-full rounded-lg backdrop:backdrop-blur-lg dark:bg-black dark:border-neutral-500 dark:border',
  ...
  def header_nav_item(url, **options, &block)
    ...
    options[:class] =
      "flex flex-row items-center gap-x-2 font-mono font-semibold uppercase text-indigo-600 dark:text-yellow-500 #{options[:class]}"
